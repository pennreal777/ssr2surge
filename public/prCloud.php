...
function get_proxy_group($proxy_names) {
	echo "[Proxy Group]".PHP_EOL;
	$i = 0;
	foreach ($proxy_names as $proxy_name){
		$tmps =  $_GET[$proxy_name];
		$url = "";
		foreach ($tmps as $tmp){
			$url= $url.",".$tmp;
		}
		...
		if ($proxy_name == "Apple") {
			...
			echo "$proxy_name = select,Proxy$url".PHP_EOL;
		}
	}
}
...
RULE-SET,https://raw.githubusercontent.com/PennReal/ssr2surge/master/public/list/China.list ,DIRECT
RULE-SET,https://raw.githubusercontent.com/PennReal/ssr2surge/master/public/list/HBO.list ,HBO
RULE-SET,https://raw.githubusercontent.com/PennReal/ssr2surge/master/public/list/Hulu.list ,Hulu
